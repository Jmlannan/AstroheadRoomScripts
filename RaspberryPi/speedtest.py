...
import math
import os
os.system('modprobe i2c_bcm2708 baudrate=400000')
def lev(V):
...
try:
	if mode == 1:
					...
					while(switch==position):
						if (time.time()>switchtime):
							if position:
								...
								dac.raw_value = xpoint
								dac.raw_value = xpoint
								dacY.raw_value = ypoint
								dacY.raw_value = ypoint
							else:
								with open('writtenvoltageX.txt','w') as f:
									f.write(str(0))
		...
		pointY = lev(centerY)
		pointStatic = pointX
		dac.raw_value = pointStatic
		#dacY.raw_value(pointStatic)
		with open('writtenvoltageX.txt','w') as f:
		...
		while(True):
			...
			while time.time() < timer:
				...
				dacY.raw_value = list[point]
				#dac.raw_value(list[point])
		...
		pattern = open('ncsuraster.txt','r')
		patternlist = eval(pattern.read())
		patternraster = [[int(int(a[0])/2),int(int(a[1]/2))] for a in patternlist]
		while (True):
			for pair in patternraster:
				dac.raw_value = pair[0]
				dacY.raw_value = pair[1]
	elif mode == 4:
		...
		while (True):
			timer = time.time()
			#for pair in l:
			#	dac.raw_value(lev(pair[0]))
			#	dacY.raw_value(lev(pair[1]))
			#	#print "%.7f" % time.time(
		...
		#set to single point
		dac.raw_value = lev(centerX)
		dacY.raw_value = lev(centerY)
		while True:
			time.sleep(1)
	elif mode == 6:
		#periody = 5017.34/3.0
		...
		while(True):
			...
			#dacY.raw_value(listY[pointy])
			#dac.raw_value(listX[pointx])
			# switched so taht x direction is slow
			dac.raw_value = listY[pointy]
			dacY.raw_value = listX[pointx]
			current_time = time.time()
		...
		while True:
			begin = time.time()
			dac.raw_value = centerLevX
			dacY.raw_value = centerLevY
			while time.time()-begin < litTime:
				time.sleep(.0001)
			dac.raw_value = dumpLevXY
			dacY.raw_value = centerLevY
		...
		while(True):
			begin = time.time()
			timer= time.time() +period
			dac.raw_value = pointX
			#dacY.raw_value(pointY)
			file_write_timer = begin + 5
			while time.time() < timer:
				point = int((time.time()-begin)/t_interval) % numpts
				dacY.raw_value = list[point]
